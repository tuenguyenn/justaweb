...
{   
    ...
    public function getAttribute(Request $request) {
        ...
        $attributes = $this->attributeRepository
            ->searchAttributes($payload['search'], $payload['option'], $this->language);
    
        // Ánh xạ từng bản ghi
        $attributeMapped = $attributes->map(function ($attribute) {
            $firstLanguage = $attribute->attribute_language->first();
            return [
                'id' => $attribute->id,
                'text' => $firstLanguage ? $firstLanguage->name : null,
            ];
        })->all();
