...
<div class="col-lg-4 bg-white border " >
    ...
    <div class="mb-4 pb-2">
        ...
        <div data-mdb-input-init class="form-outline form-white col-12">
            ...
            <select name="{module}_catalogue_id"  class="form-control">
    ...
    <div class="mb-3">
        ...
        <span class="image img-cover image-target"><img src="{{old('image',(${module}->image) ?? 'backend/img/no-image-icon.jpg')}}" alt=""> </span>
        ...
        <div data-mdb-input-init class="form-outline form-white">
            ...
            <select name="publish" class="form-control">
                @foreach(config('apps.general.publish') as $key => $val)
                    <option value="{{$key}}" {{ old('publish', ${module}->publish ?? '') == $key ? 'selected' : '' }}>
                        {{$val}}
                    </option>
                @endforeach
            </select>
        ...
        <div data-mdb-input-init class="form-outline form-white">
            ...
            <select name="follow" class="form-control">
                @foreach(config('apps.general.follow') as $key => $val)
                    <option value="{{$key}}" {{ old('follow', ${module}->follow ?? '') == $key ? 'selected' : '' }}>
                        {{$val}}
                    </option>
                @endforeach
            </select>
